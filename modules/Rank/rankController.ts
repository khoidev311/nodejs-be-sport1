...
const getRanks = async (req: Request, res: Response) => {
  try {
    ...
    const scores = await RankModel.find({...filter}).sort(sort).populate({path:"team",model:"Team"}).populate({path:"league", model:"League"});
    res.status(200).json({
        data: scores,
        meta: {
          total: size(scores),
...
const getRankByLeagueId = async (req: Request, res: Response) => {
  try {
    const { filter , sort } = queryBuilder(req);
    const { id } = req.params;
    let scores = await RankModel.find({...filter}).sort(sort).populate({path:"team",model:"Team"}).populate({path:"league", model:"League", match: {_id:id}});
